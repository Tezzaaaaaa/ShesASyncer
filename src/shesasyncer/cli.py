"""Command-line test harness for real ShesASyncer alignment runs."""
...
def _read_lyrics(path: Path) -> list[str]:
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        ...
        if not text:
            continue
        # Accept LRC without requiring a separate conversion step.
...
def _write_lrc(result, path: Path) -> None:
    def stamp(seconds):
...
def _write_html(result, path: Path) -> None:
    rows = []
    for item in result.lines:
        ...
        rows.append(f"<tr><td>{item['index']}</td><td>{start}</td><td>{end}</td><td>{confidence}</td><td>{item['source'] or 'unresolved'}</td><td>{_escape(item['text'])}</td></tr>")
    html = """<!doctype html><meta charset='utf-8'><title>ShesASyncer alignment</title>
<style>body{font:15px system-ui;margin:32px;max-width:1200px}table{border-collapse:collapse;width:100%}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{position:sticky;top:0;background:#fff}.low{opacity:.65}</style>
...
""".format(confidence=result.confidence, rows="".join(rows))
...
def align(args: argparse.Namespace) -> int:
    ...
    if not model.exists() or not vocab.exists():
        raise FileNotFoundError("Model assets not found. Run `python -m shesasyncer model setup` first, then pass --model and --vocab.")
    ...
    try:
        g2p = EspeakG2P()
        runner = OnnxCtcRunner(str(model), str(vocab), blank_token="<pad>")
        engine = CtcSingingEngine(g2p, runner)
        result = engine.align(audio_path, lyric_lines, args.language)
        # Keep the CLI result intentionally simple and machine-readable.
        payload = {
            "engine": "shesasyncer-ctc",
            "audio": str(audio),
            "lyrics": str(lyrics),
        ...
        _write_lrc(result, output.with_suffix(".lrc"))
        _write_html(result, output.with_suffix(".html"))
        print(f"JSON: {output}")
